...
@php
    ...
    $orderDate = strftime('%Y-%m-%dT%H:%M:%S', strtotime($orderfixhardware->orderDate));
...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-3">
            ...
            <textarea id="ketID" class="form-control" name="keterangan" cols="40" rows="5" readonly>{{$orderfixhardware->keterangan}}</textarea>
        </div>
    ...
    @endif
    <form action="{{$orderfixhardware->id}}" method="POST">
        @method('patch')
        @csrf
        <div class="row">
            ...
            <div class="col-sm-12">
                ...
                <textarea class="form-control" name="statusDetail" id="" cols="30" rows="5" @if ($orderfixhardware->status == 'FINISHED') readonly @endif >{{$orderfixhardware->statusDetail}}</textarea>
            </div>
        </div>

        <br>

        @if ($orderfixhardware->status != 'FINISHED')
    ...
    <div class="row">
        <div class="col-sm-1">
            <a href="/todofixhardware" class="btn btn-dark">Back</a>        
        </div>

        @if ($orderfixhardware->status == 'IN PROGRESS')
            <div class="col-sm-1">
                <form action="{{$orderfixhardware->id}}/finish" method="post" class="form-inline">
                    @method('patch')
                    @csrf
