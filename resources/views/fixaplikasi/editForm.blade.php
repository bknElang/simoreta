...
@php
    ...
    $orderDate = strftime('%Y-%m-%dT%H:%M:%S', strtotime($orderfixaplikasi->orderDate));
...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-3">
            ...
            <textarea id="ketID" class="form-control" name="keterangan" cols="40" rows="5" readonly>{{$orderfixaplikasi->keterangan}}</textarea>
        </div>
    ...
    @endif
    <form action="{{$orderfixaplikasi->id}}" method="POST">
        @method('patch')
        @csrf
        <div class="row">
            ...
            <div class="col-sm-12">
                ...
                <textarea class="form-control" name="statusDetail" id="" cols="30" rows="5" @if ($orderfixaplikasi->status == 'FINISHED') readonly @endif >{{$orderfixaplikasi->statusDetail}}</textarea>
            </div>
        </div>

        <br>

        @if ($orderfixaplikasi->status != 'FINISHED')
    ...
    <div class="row">
        <div class="col-sm-1">
            <a href="/todofixaplikasi" class="btn btn-dark">Back</a>        
        </div>

        @if ($orderfixaplikasi->status == 'IN PROGRESS')
            <div class="col-sm-1">
                <form action="{{$orderfixaplikasi->id}}/finish" method="post" class="form-inline">
                    @method('patch')
                    @csrf
