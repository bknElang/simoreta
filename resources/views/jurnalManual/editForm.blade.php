...
@section('content')
    ...
    <form action="{{$jurnalManual->id}}" method="POST">
        @method('patch')
        @csrf
        <div class="row">
            <div class="col-sm-12">
                <label for="">Status Detail</label>
                <textarea class="form-control" name="statusDetail" id="" cols="30" rows="5" @if ($jurnalManual->status == 'FINISHED') readonly @endif >{{$jurnalManual->statusDetail}}</textarea>
            </div>
        </div>

        <br>

        @if ($jurnalManual->status != 'FINISHED')
    ...
    <div class="row">
        ...
        @if ($jurnalManual->status == 'IN PROGRESS')
            <div class="col-sm-1">
                <form action="{{$jurnalManual->id}}/finish" method="post" class="form-inline">
                    @method('patch')
                    @csrf
